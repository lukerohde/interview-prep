...
class TestBillingUI(UITestBase, StaticLiveServerTestCase):
    ...
    def test_voice_chat_token_usage_and_balance(self):
        ...
        self.page.evaluate("""
        () => {
            const controller = document.querySelector('[data-controller="voice-chat"]')
            if (!controller) throw new Error('Voice chat controller not found')
            
            // Create a MessageEvent with data property as a string
